...
def api_search_hiring(q: str = ""):
    # Use gather_all() for 105+ sources with rotation
    if HS and hasattr(HS, 'gather_all'):
        out = HS.gather_all(q)
    else:
        out = gather(q)
    ...
    # Push to cloud (privacy-safe, only anonymized jobs)
    if HS and hasattr(HS, 'push_jobs'):
        try: HS.push_jobs(ded)
        except Exception as e: audit(f"push failed: {e}")
